...
#' X<-uniboot(data,1000)
uniboot<-function(data, B = 1000, groups=NULL, keepgroups=FALSE, size=1, HIcor=NULL, sampleframe="group"){ #Bootstrap function, returns a list of bootstrap resamples. This is uniboot2 in the sandbox.
  if(is.null(groups)){
  ...
  if(keepgroups){ #creates a new grouping variable that will be the correct length for the new dataset
    ...
    groupingVar<-do.call(c,(mapply(rep,groupingVar,lengths*size, SIMPLIFY = FALSE)))
  }
  times<-ncol(usedata)
  ...
  if(B!="sampleframe"){
    ...
    while(i<B+1){
      ...
      bootstrap<-mapply(`%*%`,bootstrap,cc,SIMPLIFY = FALSE, USE.NAMES = TRUE) #creates diagonalized bootstrap samples
      bootstrap<-lapply(bootstrap,t) #exp
      bootstrap<-mapply(`*`,bootstrap,sds,SIMPLIFY = FALSE, USE.NAMES = TRUE) #exp
      bootstrap<-mapply(`+`, bootstrap, means,SIMPLIFY = FALSE, USE.NAMES = TRUE) #exp
      bootstrap<-lapply(bootstrap,t) #exp
    ...
    if(sampleframe=="group"){
      ...
      samples<-mapply(`%*%`,samples,cc,SIMPLIFY = FALSE, USE.NAMES = TRUE)
      ...
      samples<-mapply(`%*%`,samples,cc,SIMPLIFY = FALSE)
    }
  }
  return(samples)
}
