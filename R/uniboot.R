...
#' X<-uniboot(data,1000)
uniboot<-function(data, B = 1000, groups=NULL, keepgroups=F, size=1, HIcor=NULL, sampleframe="group", use="everything"){ #Bootstrap function, returns a list of bootstrap resamples. This is uniboot2 in the sandbox.
  if(is.null(groups)){
  ...
  if(!is.null(HIcor)){
    ...
    if(sampleframe=="group"){
      cc<-lapply(usedata,cholcors, use=use)
    }else{
      cc<-by(usedata,groups,cholcors, use=use) #Creates a list of cholesky decompositions of the correlation matrices for each group
    }
  }
  samples<-list()
  if(B!="sampleframe"){
