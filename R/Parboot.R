...
#' X<-TestData()
#' Y<-Parboot(data=X)
Parboot<-function(X, data, groups=NULL, keepgroups=FALSE, size=1, HIcor=NULL){ #Creates a single univariate bootstrap sample for parallel bootstrapping
  if (is.null(groups)){
  ...
  bootstrap<-do.call(rbind,mapply(`%*%`,bootstrap,cc,SIMPLIFY = FALSE)) #creates diagonalized bootstrap sample
  if(keepgroups){
